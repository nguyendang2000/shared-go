...
func (inst *Service) FindOne(dbName, collectionName string, query *Query, result any, projection ...*Projection) error {
...
// If no document matches the query, ErrDocumentNotFound is returned.
// It uses the timeout defined in the Service struct to create a context for the operation.
func (inst *Service) FindOneAndDelete(dbName, collectionName string, query *Query, result any, projection ...*Projection) error {
	// Create a context with the timeout from the Service struct.
...
// If no document matches the query, ErrDocumentNotFound is returned.
// It uses the timeout defined in the Service struct to create a context for the operation.
func (inst *Service) FindOneAndReplace(dbName, collectionName string, query *Query, result any, replacement any, projection ...*Projection) error {
	// Create a context with the timeout from the Service struct.
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(inst.timeout)*time.Second)
	defer cancel()
...
// If no document matches the query, ErrDocumentNotFound is returned.
// It uses the timeout defined in the Service struct to create a context for the operation.
func (inst *Service) FindOneAndUpdate(dbName, collectionName string, query *Query, update *Query, result any, projection ...*Projection) error {
	// Create a context with the timeout from the Service struct.
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(inst.timeout)*time.Second)
	defer cancel()
...
func (inst *Service) FindMany(dbName, collectionName string, query *Query, limit int64, offset int64, sort []string, result any, projection ...*Projection) error {
	// Create a context with the timeout from the Service struct.
	ctx, cancel := context.WithTimeout(inst.context, time.Duration(inst.timeout)*time.Second)
	defer cancel()
...
func (inst *Service) FindAll(dbName, collectionName string, query *Query, sort []string, batchSize int64, result any, projection ...*Projection) error {
